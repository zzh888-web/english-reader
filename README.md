...
- **API 密钥按接口地址分别记忆**：给 A 提供商填过密钥后，切到 B 再切回 A，A 的密钥和模型会自动填回，不用重新粘贴；密钥框右侧的 👁 按钮可显示/隐藏密钥便于核对。
- 模型名可手填，或点 **Fetch models** 自动拉取列表；**Test connection** 可一键测试连通性。
- API Key 只保存在本机浏览器 localStorage，仅通过本地代理发给你填写的 Base URL，不经过任何第三方。
- **温度（Temperature）**：控制模型输出的"随机/发散"程度，滑条范围 0～2（多数模型建议 0～1）。**调低（0～0.3）**：回答更稳定、保守、贴事实，适合词汇辨析、语法答疑、背景考证；**调高（0.7～1.0+）**：表达更多样、有想象力，适合文学赏析、开放式讨论。默认 0.7。两项保障：**① 若提供商拒绝温度值（如 Kimi 思考系列仅支持固定温度），本地代理会自动去掉该参数重试一次，对话不中断；② 勾选「跟随模型默认」则完全不发送温度参数**，由模型用自身默认值。另外，**句子翻译内部固定用 0.2 的低温**以保证译文稳定（勾选「跟随模型默认」后同样不发送）。
