...
浏览器会自动打开 `http://127.0.0.1:8765`（端口被占用时自动顺延）。关闭服务窗口或按 Ctrl+C 即停止。**首次使用请在右上角 ⚙ Settings 里填入大模型 API。**
...
- 其他选项：**Assistant replies in**（AI 解释与对话的回复语言，默认 Chinese，可改 English 等）；**Translate into**（翻译目标语言，默认 Chinese，可改日/韩/法/德/西）。
- **Explanation prompt（解释提示词，可选）**：自定义"解释一句话"时发给模型的指令模板。占位符 `{TEXT}` = 选中的句子原文，`{CONTEXT}` = 所在段落。留空则使用内置提示词。例如想让解释更简短：`用中文简明解释下面这句英文的意思和难点，不超过三句话：{TEXT}`。单词查词走独立的内置提示词，不受此项影响。
- **Tavily API key（可选）**：给 🔍 联网搜索用的 Key。**本机若装有 Tavily CLI（`~/.tavily/config.json`）会自动探测、无需填写**；没填也没关系，自动降级为免 Key 的必应网页搜索。
...
3. **解释**：右键 → `Explain`，解释内容进入右侧对话栏。被解释的英文原句以**深色卡片、白色文字**醒目展示（"EXPLAINING THIS"），一眼可见在解释哪句；**默认用中文解释**（Settings 里 "Assistant replies in" 可改）。选中单个单词后 Explain 会按词典方式讲解（音标、词性、语境义、例句）。
